...
- **Calibration ritual** (Settings, ~20s, per device profile): play along with 8 metronome clicks; median offset = the profile's latency correction; spread = the profile's jitter, which widens grading windows automatically.
...
**v1 — self-paced matcher (rehearsal mode, no grid):** sequence alignment on pitch order only (greedy with 1-note lookahead for v1); IOI outliers logged as hesitations relative to the local median IOI; backward jumps to previously-matched positions are detected only so **replayed material is ignored** (no false insertions) — rehearsal has no restart concept, and first-pass errors stand.
...
- Rating map (v0), **selected by the material, never by the mode label** (the matcher follows the material too, §4):
  ...
  - **Pulsed runs** (scales, arpeggio traversals, F7 lines — the pulse is part of the material): any pitch error, **or more than one out-of-window onset → Again** · exactly one out-of-window onset → **Hard** · every onset inside its grid window (§4) → **Good**. The same shape as engine-B aggregation (04 §5) — and why slow, clunky runs can never pass as fluent; gates additionally demand the anchor tempo (02 §1).
  - **Self-paced material** (unpulsed rehearsal): hesitations and IOI outliers are **diagnostic-only, never rating** (§1.3); pitch errors rate as discrete answers do; latency decides Hard vs Good.
